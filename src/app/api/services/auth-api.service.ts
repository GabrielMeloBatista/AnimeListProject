...
import { CredencialRegisterDto } from '../models/credencial-register-dto';
...
export class AuthApiService extends BaseService {
   ...
   */
  static readonly CreateUserPath = '/api/v1/auth/register';
  ...
  createUser$Response(params: {
    body: CredencialRegisterDto
  },
  context?: HttpContext

): Observable<StrictHttpResponse<Array<CredencialDto>>> {

    const rb = new RequestBuilder(this.rootUrl, AuthApiService.CreateUserPath, 'post');
    if (params) {
      rb.body(params.body, 'application/json');
    }
